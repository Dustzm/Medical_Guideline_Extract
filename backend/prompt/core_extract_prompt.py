...
- **推荐意见抽取**：
    -一条推荐意见的推荐药物可能有多个，抽取出每个推荐药物的推荐强度和证据质量等级，一一对应
    -例如：推荐意见中描述为"推荐使用药物A(1B)、药物B(2C)"，则推荐强度为"强推荐，弱推荐"，证据质量等级为"中等质量，低质量"
- **证据抽取注意事项**：
...
10. 输出时按照以下输出顺序，每个临床问题后输出推荐意见和治疗方案，每个推荐意见后输出对应的证据，请注意在输出时不得遗漏，根据原文内容严格输出每个临床问题下的推荐意见、推荐意见下的证据，不得遗漏，以下仅为示例，在正式输出时需要注意数量：
    - 临床问题1
      - 推荐意见1-1
      - 治疗方案1-1
        - 证据1-1-1
